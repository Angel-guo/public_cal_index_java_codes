...
@Data
@AllArgsConstructor
public class CalIndexMapHandlerCallable implements Callable {
    private static final Logger logger = LoggerFactory.getLogger(CalIndexMapHandlerCallable.class);
    ...
    public Object call() throws Exception {
        while (true) {
            ...
            if (map.size() > 0) {
                logger.info("TASK MAP SIZE is :{} ", map.size());
                for (String s : map.keySet()) {
                    if (map.get(s).getStatus() != -1) {
                        ...
                        for (Future<List<String>> future : statusFutures.getFutures()) {
                            ...
                            logger.info("CalIndexMapHandlerCallable's fileName:{} ", fileName);
