...
//                                    resultMap.put(splitArray[xLocationColumn], dataList1);
    ...
    public List<CarBatteryVo> getBatteryList() {
        ...
        logger.info("get all BatteryList: {}",JSONObject.toJSON(batteryList));
    ...
    public List<CarBatteryVo> getAutomobileEnterpriseByBattery(String battery) {
        List<CarBatteryMap> resultList = carBatteryMapMapper.getAutomobileEnterpriseByBattery(battery);
        logger.info("get AutomobileEnterprise by battery: {}",JSONObject.toJSON(resultList));
        if(CollectionUtils.isEmpty(resultList)){return new ArrayList<>();}
        // 拼接成对应的形式并返回
        return resultList.stream().map(obj->new CarBatteryVo(obj.getId(),obj.getCar(),
    ...
    public String getRecordListByCalIndex(String calIndex, HttpServletResponse resp) {
        logger.info("getRecordListByCalIndex method parameters, calIndex : {}", calIndex);
        List<CalIndexRecord> recordList = calIndexRecordMapper.getRecordListByCalIndex(calIndex);
        if(CollectionUtils.isEmpty(recordList)){
            return "this cal index has no latest record, please force query!";
        }
        String csvFilePath = FILE_PATH.concat("/calIndexCSV/").concat(calIndex).concat(".csv");
        BufferedWriter fileWriter = null;
        try {
            fileWriter = new BufferedWriter(new FileWriter(csvFilePath));
            // 写入表头
            String header = String.format("%s,%s,%s,%s,%s",
                    "cal_index", "cal_vars", "result", "status", "create_time");
            logger.info("getRecordListByCalIndex write header: {}", header);
            fileWriter.write(header);

            // 写入内容
            for (CalIndexRecord obj : recordList) {
                String line = String.format("%s,%s,%s,%s,%s",
                        obj.getCalIndex(), obj.getCalVars(), obj.getResult(), obj.getStatus(), obj.getCreateTime());
                fileWriter.newLine();
                fileWriter.write(line);
            }
            logger.info("getRecordListByCalIndex write success : >>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
            logger.info("getRecordListByCalIndex save path : >>>>>>>>>>>>>>>>>>>>>>>>>>>>> {}", FILE_PATH);
            // 关闭流
            fileWriter.close();
        } catch (Exception e) {
            e.printStackTrace();
        }
        logger.info("invoke downloadFile method download file : >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        downloadFile(calIndex.concat(".csv"), resp, new File(csvFilePath));
        return "download this cal_index latest record success！";
    }
